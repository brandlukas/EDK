			...
			<tbody>
				{foreach from=$loop item=a key=pilot}
					{foreach from=$a item=i key=b}
						<tr{if $i.destroyed} class="destroyed"{/if} onclick="window.location.href='{$i.kll_url}';">
							...
							{else}
								...
								{/if}
							<td>
								<a href="{$i.crp_url}">{$i.corp|truncate:25:"...":true}</a><br/>
								<a href="{$i.all_url}" class="kill-related-subsection">{$i.alliance|truncate:25:"..":true}</a>
							</td>
						</tr>
					{/foreach}
				{/foreach}
