					...
					{if $k.loss}
						...
						{if isset($cynos) || isset($cloaks)}
							<td class="kl-cyno-cloak-mod{if isset($cloaks)} cloak{/if}{if isset($cynos)} cyno{/if}">
								{if isset($cloaks)} 
									<img src=http://image.eveonline.com/InventoryType/11370_32.png {if in_array({$k.id}, $cloaks)}class="fitted"{/if} alt="" />
								{/if}
								{if isset($cynos)}
									<img src=http://image.eveonline.com/InventoryType/21096_32.png {if in_array({$k.id}, $cynos)}class="fitted"{/if} alt="" />
								{/if}
							</td>
						{/if}
