<?php
function handle_involved($kill, $side, &$pilots, $completeInformation = FALSE)
{
    ...
    $sql="select ind_plt_id AS ind_plt_id, ind_crp_id, ind_all_id, ind_sec_status, ind_shp_id, ind_wep_id, ind_dmgdone,
            typeName, plt_name, crp_name, all_name, scl_points, scl_id
            from kb3_inv_detail
    ...
    while ($row=$qry->getRow())
    {
        if(config::get('fleet_battles_mod_sideassign'))
        {
            ...
            $sideAssignmentQuery = DBFactory::getDBQuery();
            $killTimestamp = $kill->getTimeStamp();
            $sql = "SELECT
                            side
                        FROM kb3_side_assignment
                        WHERE system_id = ".$kill->getSystem()->getID()."
                            AND timestamp_start <= '".$killTimestamp."'
                            AND timestamp_end >= '".$killTimestamp."'
                            AND entity_id = ".$entityId."
                            AND entity_type = '".$entityType."'
            ";

            $sideAssignmentQuery->execute($sql);
            $sideAssignment = $sideAssignmentQuery->getRow();
            
            if($sideAssignment)
            {
                $pilotSide = $sideAssignment["side"];
            }
     ...
     * @return <type>
     */
function handle_destroyed($kill, $side, &$destroyed, &$pilots, $completeInformation = FALSE)
    {
