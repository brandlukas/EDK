...
class Battle
{
	...
	function validate()
	{
         ...
         */
        public static function updateCacheForBattle($battleId, $killIsk, $lossIsk, $kills, $losses, $involved, $timestampStart, $timestampEnd)
        {
...
class BattleListTable
{
	...
	function getTableStats()
	{
		...
		{   
			...
			$battle = array_merge(array('name' => $contract->getName(), 'startdate' => $contract->getStartDate(), 'enddate' => $contract->getEndDate(),
				...
				'kll_id' => $contract->getKillID(), 'id' => $contract->getID()), $kdata, $ldata);
		...
		if (config::get('fleet_battles_mod_cache'))
		{
			$sql = "select * from kb3_battles_cache where 1 order by end desc";
			$cacheq = new DBQuery();
			$cacheq->execute($sql);
			...
			while($cb = $cacheq->getRow())
			{
				...
				$tbldata[] = array('name' => $cb['system'], 'kll_id' => $cb['kll_id'], 'id' => $cb['battle_id'], 'enddate' => $cb['end'],
					...
					'killisk' => $cb['killisk'], 'lossisk' => $cb['lossisk'], 'bar' => $cb['bar'], 'battle_url' => edkURI::build($args, array('kll_id', $cb['kll_id'], true), array('battle', true, true)));
			}
		}
		return $tbldata;
	}
	...
	function cacheBattle($battle)
	{
		...
		$sql .= $battle['name']."','".$battle['startdate']."','".$battle['enddate']."')";
	...
	function generate()
	{
		if ($table = $this->getTableStats())
		{
			...
			else
			{
				...
				$statsql = 'select COUNT(*) as tbattles, SUM(involved) as ainvolved,
				SUM(losses) as alosses, SUM(kills) as akills,
				SUM(killisk) as akillisk, SUM(lossisk) as alossisk
				from kb3_battles_cache';
				$statq = new DBQuery();
			...
			return $smarty->fetch(dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'template' . DIRECTORY_SEPARATOR . 'battlelisttable.tpl');
		}
	}
}